...
def main():
    ...
    log.add_argument('--raw_file',
                       ...
                       help="Raw Log File",
                       action="store")
    md_log_file = '.'.join([startup_ts, 'md'])
    log.add_argument('--md_file',
                       dest='md_file',
                       type=str,
                       default=md_log_file,
                       help="Metadata Log File",
                       action="store")
    ...
    print '      writing KISS to:', fp_raw
    fp_md = '/'.join([args.log_path,args.md_file])
    print '  writing Metadata to:', fp_md
        ...
        try:
            ...
            if packet: #packet received, lets parse it!
                ...
                if ser_data[0]==FEND and ser_data[-1]==FEND: #KISS Frame received
                    ...
                    ax25 = {}
                    ax25_count += 1
                    ax25['packet_id'] = ax25_count
                    ax25['packet_count'] = packet_count
                    ax25['ts_utc'] = ts_str
                    ...
                    ax25_json = json.dumps(ax25_frames, indent=4)
                    print ax25_json
                    ...
                    #
                    # #just a check to make sure file is in right format for reading
                    # try:
                    #     with open(fp_md, 'r') as in_f:
                    #         x = json.load(in_f)
                    #         print x
                    #     in_f.close()
                    #     print "JSON valid"
            ...
            for i,frame in enumerate(ax25_frames):
                print i, frame
            sys.exit()
        time.sleep(0.01) #needed to throttle for CPU
